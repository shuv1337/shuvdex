...
**Default aesthetic: Blueprint.** Unless the content strongly calls for a different direction (e.g., prose-heavy essays → Editorial, informal/approachable → Paper/ink), start with Blueprint. It's the house style — technical, precise, information-dense, and distinctly non-generic. Key traits:
- **Dark-first** (`#0c1220` base) with proper light mode alternate
- **Dot-grid background** (`radial-gradient(circle, rgba(56,189,248,0.04) 1px, transparent 1px); background-size: 20px 20px`)
- **Monospace-forward** — IBM Plex Mono or JetBrains Mono for labels, headers, and section markers; IBM Plex Sans or DM Sans for body text
- **Slate/cyan palette** — `#0ea5e9`, `#38bdf8`, `#0891b2` as primary accents; `#059669` (emerald) and `#fbbf24` (amber) as secondaries
- **Numbered section badges** with bordered counters (`[01]`, `[02]`) instead of colored dots
- **Precise borders** — `rgba(56,189,248,0.12)` card borders, no rounded-pill shapes, tight 6-8px border-radius
- **Corner annotations** — small `BRIEF`, `REF`, `SRC` labels positioned absolute top-right of cards
- **File-reference style citations** — `src: chapter-01.md:L14` instead of `— Chapter 1`
- See `./templates/blueprint-default.html` for the complete reference implementation

**Constrained aesthetics (alternate choices):**
...
Vary the choice when there's a reason. If the last diagram was Blueprint and the user asks for another explainer in the same session, consider Editorial or Paper/ink for contrast. But Blueprint is the default starting point, not one option among equals. The swap test: if you replaced your styling with a generic dark theme and nobody would notice the difference, you haven't designed anything.
...
- IBM Plex Sans + IBM Plex Mono (reliable, readable — **Blueprint default**)
...
- Slate/cyan + emerald/amber (`#0ea5e9`/`#38bdf8`, `#059669`, `#fbbf24`) — **Blueprint default**, technical/precise
...
```css
/* Dark-first (blueprint default, IDE-inspired, terminal): */
:root { /* dark values */ }
@media (prefers-color-scheme: light) { :root { /* light values */ } }

/* Light-first (editorial, paper/ink): */
:root { /* light values */ }
